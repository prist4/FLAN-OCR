...
void cuttedSurface(SDL_Surface *img, int firstCut,int lastCut, struct Neuronal_Network *net)
{
    /* Variables */
    Uint32 pixel;
    ...
    copy = SDL_CreateRGBSurface(SDL_HWSURFACE,img -> w,lastCut - firstCut,img -> format -> BitsPerPixel, 0, 0, 0, 0);
        ...
        if(r == 255 && g == 0 && b == 0)
            ...
            for(int j = firstCut + 1; j < img -> w; j++)
            {
                ...
                if(r == 255 && g == 0 && b == 0)
                {
                    ...
                    lastCut = j - 1;
                    //Create a surface that contain the cutting zone
                    copy = SDL_CreateRGBSurface(SDL_HWSURFACE,lastCut-firstCut + 1,img->h,img->format->BitsPerPixel,0,0,0,0);
                    //Copy the cutting zone into the new surface
